...
resource "azurerm_linux_virtual_machine" "dev-vm" {
  ...
  admin_ssh_key {
    username   = "adminuser"
    public_key = file("~/.ssh/mtcazureke.pub") # Pass the file path as recorded in previous step
  }
  os_disk {
    caching              = "ReadWrite"
...
output "public_ip_address" {
  ...
  value = "${azurerm_linux_virtual_machine.dev-vm.name}: ${azurerm_public_ip.dev-ip.ip_address}"
}
